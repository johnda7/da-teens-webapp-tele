...
import { boundariesParentModule } from '@/data/boundariesParentModule'
...
export default function ParentBoundariesModule({ onBack }: Props) {
  ...
  const [progress, setProgress] = useState<ProgressData>(() => {
    ...
    const saved = localStorage.getItem('boundaries-parent-progress')
    if (saved) {
      return JSON.parse(saved)
    }
    return {
      completedLessons: [],
      currentLesson: boundariesParentModule.lessons[0].id,
      xpEarned: 0,
      skillsUnlocked: 0,
      streak: 0,
      totalLessons: boundariesParentModule.lessons.length,
      checkIns: []
    }
  })
  ...
  useEffect(() => {
    localStorage.setItem('boundaries-parent-progress', JSON.stringify(progress))
  }, [progress])

  const handleStartLearning = () => {
    // Find first incomplete lesson
    const firstIncomplete = boundariesParentModule.lessons.find(
  ...
  const handleLessonComplete = (lessonId: string, xpEarned: number) => {
    setProgress(prev => {
      ...
      const lessonIndex = boundariesParentModule.lessons.findIndex(l => l.id === lessonId)
      const nextLesson = boundariesParentModule.lessons[lessonIndex + 1]
  ...
  const getWeekProgress = () => {
    ...
    const week1Lessons = boundariesParentModule.lessons.slice(0, 1)
    const week2Lessons = boundariesParentModule.lessons.slice(1, 3)
    const week3Lessons = boundariesParentModule.lessons.slice(3, 5)
  ...
  const getCurrentWeekLessons = () => {
    const lessonsPerWeek = currentWeek === 1 ? 1 : 2
    const startIndex = currentWeek === 1 ? 0 : currentWeek === 2 ? 1 : 3
    return boundariesParentModule.lessons.slice(startIndex, startIndex + lessonsPerWeek)
  }
  ...
  if (selectedLesson) {
    const lesson = boundariesParentModule.lessons.find(l => l.id === selectedLesson)
            ...
            <AdaptiveRecommendation
              ...
              onRecommendationSelect={(rec) => {
                if (rec.type === 'lesson') {
                  ...
                  const lesson = boundariesParentModule.lessons.find(l => l.title.includes(rec.title.split(':')[1]?.trim()))
                  if (lesson) {
