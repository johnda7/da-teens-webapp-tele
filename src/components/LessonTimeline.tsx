...
const LessonTimeline = memo(function LessonTimeline({ 
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'current':
        return <Play size={20} weight="fill" className="text-blue-500" />
      case 'available':
        return <Circle size={20} weight="bold" className="text-blue-400" />
      default:
        return <Lock size={20} weight="fill" className="text-gray-400" />
    }
  }
  ...
  const getDifficultyBadge = (difficulty: Lesson['difficulty']) => {
    ...
    return (
      <Badge variant="outline" className={`${config.color} text-[8px] px-1 py-0`}>
        {config.label}
      </Badge>
    )
  }
  ...
  return (
    <div className="space-y-2">
      <div className="relative">
        {/* Vertical connecting line - синяя - компактнее */}
        <div className="absolute left-5 top-0 bottom-0 w-0.5 bg-gradient-to-b from-blue-200 via-blue-100 to-gray-200" />
                    ...
                    <CardContent className="p-2">
                      ...
                      <div className="relative">
                        <div className="flex items-center justify-between mb-0.5">
                          ...
                          {status === 'current' && (
                              <Badge className="bg-blue-600 text-white text-[8px] border-0 px-1 py-0">
                              Текущий
                            </Badge>
                          )}
                          {status === 'completed' && (
                              <Badge className="bg-green-600 text-white text-[8px] border-0 px-1 py-0">
                                ✓
                            </Badge>
                          )}
                          </div>
                        ...
                        <h3 className="text-xs font-bold text-gray-900 mb-0.5 line-clamp-1">
                          {lesson.title}
                        </h3>
                        <p className="text-[9px] text-gray-600 mb-1 line-clamp-1">
                          {lesson.subtitle}
                        </p>
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-0.5">
                            ...
                            <div className="flex items-center gap-0.5">
                              <Lightning size={8} className="text-orange-500" weight="fill" />
