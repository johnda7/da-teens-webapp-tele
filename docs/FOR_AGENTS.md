...
**Готово!** Сервер запустится на свободном порту (обычно 5000 или 5001).

### 📋 КАК УЗНАТЬ ПОРТ?

После `npm run dev` в консоли будет:
```
➜  Local:   http://localhost:5001/da-teens-webapp-tele/
```

**Просто открой этот URL!**

### ⚠️ ВАЖНО: ПОЧЕМУ НЕ ЛЮБОЙ ПОРТ?

Dev‑сервер Vite использует свободный порт (по умолчанию 5174).
- Если 5000 занят → автоматически выбирает 5001, 5002 и т.д.
Персистентность состояния реализована локально (localStorage `useKV`).

### 🐛 ПРОБЛЕМЫ?

**Смотри:** `docs/LOCAL_SETUP.md` - полная инструкция с диагностикой

**Быстрое решение:**
```bash
pkill -9 -f vite && rm -rf node_modules/.vite && npm run dev
```
...
## 🎮 КРИТИЧНО: ИНТЕРАКТИВНОСТЬ ДЛЯ ПОДРОСТКОВ

**⚠️ ГЛАВНОЕ ПРАВИЛО:** Подросткам **НЕ интересно** просто читать текст!
...
- 🖼️ Картинки, анимации, визуализации
...
- ⏱️ 5-15 минут максимум на урок
- 📚 Разбивай на микро-блоки
- ⏯️ Можно остановиться и вернуться
...
- ✅ Интерактивная карта "5 кругов близости" с drag-and-drop
- ✅ 8 примеров из жизни с цветной визуализацией
- ✅ Изображения в тексте (Unsplash)
- ✅ Градиентные подписи
- ✅ Badge за завершение

**УРОК 2: "Признаки нарушенных границ"**
- ⏳ Body Scan Tool (планируется)
...
2. **Затем:** Открыть URL из консоли (обычно `http://localhost:5001/da-teens-webapp-tele/`)
...
# Сервер автоматически выберет свободный порт (обычно 5000 или 5001)
# В консоли будет строка с URL - просто открой его!
# Пример: ➜  Local:   http://localhost:5001/da-teens-webapp-tele/
```

**❌ ЗАПРЕЩЕНО деплоить без предварительного просмотра локальной версии!**

**📖 Подробнее:** `docs/LOCAL_SETUP.md` - полная инструкция по локальному запуску

---

### 🎯 Правило #0.8: TELEGRAM MINI APP - МИРОВЫЕ ЛУЧШИЕ ПРАКТИКИ
...
### ⚠️ **ИЗВЕСТНЫЕ ОШИБКИ И КАК ИХ ИЗБЕЖАТЬ**

#### **1. Дублирование кода при merge конфликтах**

**Проблема:** При разрешении конфликтов могут появиться дубликаты кода (например, массив `LEVEL_TITLES` или функция `getLevelTitle` дважды).

**Как исправить:**
```bash
# 1. Проверить файл на дубликаты перед коммитом
grep -n "LEVEL_TITLES\|getLevelTitle" src/lib/gamification.ts

# 2. Использовать read_lints для проверки
read_lints src/lib/gamification.ts

# 3. Всегда читать весь файл перед редактированием
read_file src/lib/gamification.ts
```
...
**Пример ошибки (исправлено в коммите 3db1882):**
```typescript
// ❌ БЫЛО (дубликат):
export const gamification = new GamificationEngine()

  { level: 1, title: 'Новичок', emoji: '🌱' }, // ❌ Дубликат
  // ... весь массив повторяется
]
...
**Проблема:** Если другой процесс использует git, могут появиться `.lock` файлы.

**Как исправить:**
```bash
# Удалить lock файл вручную
rm -f .git/ORIG_HEAD.lock
# Затем повторить операцию
...
**Проблема:** Коммит прошёл локально, но GitHub Actions выдаёт ошибки TypeScript.

**Как исправить:**
```bash
# 1. Локально запустить сборку
npm run build
...
# 4. Исправить ошибки и закоммитить заново
```

**Превентивные меры:**
- ✅ Всегда запускать `npm run build` перед push
- ✅ Использовать `read_lints` для проверки
- ✅ Проверять все изменённые файлы на ошибки
...
## 🧪 ЛОКАЛЬНЫЙ ЗАПУСК: ОБЯЗАТЕЛЬНАЯ АВТО‑ПРОВЕРКА

Перед тем как говорить пользователю «сервер запущен», необходимо автоматически проверить, что страница реально отвечает 200 OK.
...
- убивает залипшие процессы Vite
- чистит кеш `node_modules/.vite`
- стартует `vite` и ждёт `Local:` в логе
- формирует корректный URL с базой: `/da-teens-webapp-tele/`
- проверяет, что `GET /da-teens-webapp-tele/` отвечает `200`
- выводит итоговые ссылки (и `duo.html`, если есть)

Пример вывода:

```
✅ Dev готов
INDEX_URL=http://localhost:5174/da-teens-webapp-tele/
DUO_URL=http://localhost:5174/da-teens-webapp-tele/duo.html (200)
```

Только после `✅ Dev готов` можно сообщать пользователю ссылку. Если код не 200 — скрипт печатает хвост лога, и нужно диагностировать локально.
...
| Practice | практика | Практическое задание |
...
```typescript
{
  ...
  "графики": "Recharts",
  "хранилище": "KV (localStorage, persistent в браузере)",
  "деплой": "GitHub Pages",
...
10. Начисление XP, проверка бейджей
   ↓
11. Прогресс сохраняется в KV (localStorage) для dev/MVP или Google Sheets (production)
   ↓
...
```typescript
// Сейчас: KV (browser localStorage)
...
- [x] **WeekTabs.tsx** - навигация по неделям с Liquid Glass дизайном
- [x] **ContentCards.tsx** - адаптивные карточки уроков с анимациями
- [x] **AdaptiveRecommendation.tsx** - AI рекомендации на основе эмоций
- [x] **SkeletonLoader.tsx** - улучшенная загрузка (Perplexity принцип)
- [x] **BoundariesModule.tsx** - интеграция всех новых компонентов
- [x] Адаптивный движок обучения с эмоциональным анализом (496 строк)
- [x] Система геймификации (480 строк)
- [x] 4 компонента вдохновлённых Google (1,560 строк)
- [x] **10 принципов Perplexity** полностью применены
- [x] **iOS 26 Liquid Glass** дизайн внедрён
- [x] Интеграция AdaptiveLessonViewer
- [x] Полная документация (5 MD файлов, 91 KB)
- [x] **Production деплой** с новыми компонентами
...
- ✅ `LOCAL_SETUP.md` - локальный запуск и диагностика (НОВЫЙ!)
- ✅ `IMPLEMENTATION_GUIDE.md` - обзор реализации
...
- ✅ `PROJECT_VISION.md` - философия проекта
...
### 🔧 2 ноября 2025, 01:00 - Исправлена портовая конфигурация

**Проблема:** Локальный сервер не запускался, конфликты портов, устаревшая документация

**Что исправлено:**
- ✅ Удалены устаревшие файлы: `QUICK_LOCAL_START.md`, `LOCAL_TROUBLESHOOTING.md`
- ✅ Создан единый файл: `LOCAL_SETUP.md` (инструкция + диагностика)
- ✅ Добавлена секция "ЛОКАЛЬНЫЙ ЗАПУСК" в начало FOR_AGENTS.md
- ✅ Обновлена информация о портах во всех документах
- ✅ Исправлены пути: `src/App.tsx` → `src/app/App.tsx`
- ✅ Обновлена информация о деплое: Vercel → GitHub Pages

**Теперь всё работает:**
- Сервер автоматически выбирает свободный порт (обычно 5000 или 5001)
- Полная диагностика в `docs/LOCAL_SETUP.md`
- Агенты не будут путаться с устаревшей информацией
...
### 📊 Статистика проекта (Декабрь 2024)

**Документация:**
- 10 актуальных .md файлов (~90 KB)
- FOR_AGENTS.md - 3,250+ строк (главный файл, обновлён)
...
- **4 новых компонента** с Perplexity принципами
- **iOS 26 Liquid Glass** дизайн внедрён
...
- **Последний коммит:** `686d215` - "🔒 Feat: скрыты модули от учеников, добавлен dev режим"

**Завершённые задачи (Декабрь 2024):**
1. ✅ **WeekTabs.tsx** - навигация по неделям с Liquid Glass дизайном
2. ✅ **ContentCards.tsx** - адаптивные карточки уроков с анимациями
3. ✅ **AdaptiveRecommendation.tsx** - AI рекомендации на основе эмоций
4. ✅ **SkeletonLoader.tsx** - улучшенная загрузка (Perplexity принцип)
5. ✅ **BoundariesModule.tsx** - интеграция всех новых компонентов
6. ✅ **10 принципов Perplexity** полностью применены
7. ✅ **iOS 26 Liquid Glass** дизайн внедрён
8. ✅ **Production деплой** с новыми компонентами
9. ✅ **FOR_AGENTS.md** обновлён с актуальной информацией
10. ✅ **Оптимизация производительности** - убраны тяжелые анимации
11. ✅ **Lazy loading** - все компоненты загружаются по требованию
12. ✅ **Dev режим** - скрыты модули от учеников, доступ только к "Личные границы"
13. ✅ **Кнопка профиля** - уменьшена до w-5 h-5 (20px)
14. ✅ **Кнопка "Назад"** - исправлена для Telegram Mini App

---

## 🔒 **DEV РЕЖИМ И УПРАВЛЕНИЕ МОДУЛЯМИ**

### **🎯 Текущая конфигурация (28.10.2024):**

**Для учеников:**
- ✅ Видят только модуль "Личные границы"
- ✅ Красивая карточка с описанием и статистикой
- ✅ Фокус на одном модуле для лучшего обучения
- ✅ Кнопка "Начать обучение" ведет в модуль

**Для разработчиков:**
- ✅ Dev режим автоматически включается на localhost
- ✅ Можно включить через `localStorage.setItem('devMode', 'true')`
- ✅ Кнопка переключения в хедере (иконка мозга)
- ✅ Badge "DEV" показывает текущий режим
...
// Или нажать кнопку мозга в хедере
```

### **📊 Статистика модуля "Личные границы":**
- **9 уроков** - полный курс
- **3 недели** - продолжительность
- **12+ лет** - целевая аудитория
- **Градиентный дизайн** - blue to purple
- **Иконка Shield** - защита границ
...
- **Dev storage:** localStorage (KV) — для MVP и разработки
...
### **🔒 Dev режим и скрытие модулей - Фокус на обучении**

**Коммит:** `686d215` - "🔒 Feat: скрыты модули от учеников, добавлен dev режим"

**Что добавлено:**

#### **1. Dev режим для разработчиков**
- ✅ **Автоматическое включение** на localhost и 127.0.0.1
- ✅ **localStorage управление** через `devMode` флаг
- ✅ **Кнопка переключения** в хедере (иконка мозга)
- ✅ **Badge "DEV"** для индикации режима

#### **2. Скрытие модулей от учеников**
- ✅ **Только "Личные границы"** доступен ученикам
- ✅ **Красивая карточка модуля** с статистикой
- ✅ **Фокус на обучении** - один модуль за раз
- ✅ **Кнопка "Начать обучение"** ведет в модуль

#### **3. Оптимизация производительности**
- ✅ **Убраны тяжелые анимации** (3 больших motion.div)
- ✅ **Lazy loading** всех компонентов
- ✅ **Статичные элементы** вместо анимированных
- ✅ **Принцип Perplexity** - скорость превыше всего

#### **4. Исправления UI**
- ✅ **Кнопка профиля** уменьшена до w-5 h-5 (20px)
- ✅ **Кнопка "Назад"** работает в Telegram Mini App
- ✅ **Импорт ArrowLeft** иконки
- ✅ **Отладочные console.log** для кнопки "Назад"

### **🎯 Предыдущие обновления - Модуль "Личные границы"**

**Коммит:** `86bcb87` - "🚀 Модуль Личные границы: Применены 10 принципов Perplexity"

**Что добавлено:**

#### **1. WeekTabs.tsx** - Навигация по неделям
- ✅ **Liquid Glass дизайн** с `backdrop-blur-[40px]`
- ✅ **Анимированные индикаторы** активной недели
- ✅ **Progress rings** для каждой недели
- ✅ **Spring physics** анимации (Framer Motion)
- ✅ **Вдохновлено Google Learn Your Way**

#### **2. ContentCards.tsx** - Карточки контента
- ✅ **Адаптивные карточки** (Video, Practice, Homework)
- ✅ **Hover анимации** с `scale: 1.02, y: -4`
- ✅ **Progress bars** для отслеживания прогресса
- ✅ **Locked/Completed состояния** с визуальными индикаторами
- ✅ **Micro-interactions** для лучшего UX

#### **3. AdaptiveRecommendation.tsx** - AI рекомендации
- ✅ **Эмоциональный анализ** на основе CheckIn данных
- ✅ **Персонализированные рекомендации** уроков
- ✅ **Priority levels** (High, Medium, Low)
- ✅ **Real-time адаптация** под настроение пользователя
- ✅ **Интеграция с Adaptive Learning Engine**

#### **4. SkeletonLoader.tsx** - Улучшенная загрузка
- ✅ **Skeleton states** для всех компонентов
- ✅ **Perceived speed** улучшение (принцип Perplexity)
- ✅ **Smooth transitions** между состояниями
- ✅ **Multiple variants** (card, text, button, avatar)

#### **5. BoundariesModule.tsx** - Интеграция всех компонентов
- ✅ **WeekTabs** навигация вверху
- ✅ **ContentCards** основной контент
- ✅ **AdaptiveRecommendation** AI рекомендации
- ✅ **Toggle для AI рекомендаций** с анимацией
- ✅ **Emotional state detection** из CheckIn данных

### **🎨 Применённые принципы Perplexity:**

1. **⚡ Speed** - Skeleton loading, оптимизированные анимации
2. **🎯 Truth-seeking** - Реальные эмоциональные данные из CheckIn
3. **🎨 Minimalism** - Чистый Liquid Glass дизайн
4. **🤖 AI-first** - AdaptiveRecommendation с эмоциональным анализом
5. **👤 User-centric** - Персонализация под настроение подростка
6. **🔄 Continuous improvement** - Адаптивные рекомендации
7. **🔍 Transparency** - Четкие progress indicators
8. **🎮 Gamification** - Progress rings, XP система
9. **🌐 Platform strategy** - Интеграция с Telegram Mini App
10. **💙 Emotion-aware** - UI адаптируется под эмоциональное состояние

### **📱 iOS 26 Liquid Glass Design:**

- ✅ **`backdrop-blur-[40px]`** - размытие фона
- ✅ **`bg-white/70`** - полупрозрачный белый фон
- ✅ **`border-white/20`** - тонкие границы
- ✅ **Spring animations** - физически корректные анимации
- ✅ **8px grid system** - консистентные отступы
- ✅ **44x44pt touch targets** - доступность

### **🔧 Технические улучшения:**

- ✅ **TypeScript типизация** для всех новых компонентов
- ✅ **Framer Motion** анимации с spring physics
- ✅ **Responsive design** для всех экранов
- ✅ **Accessibility** поддержка (ARIA labels)
- ✅ **Performance** оптимизация (lazy loading)

### **📊 Статистика обновления:**

- **Новых компонентов:** 9 (WeekTabs, ContentCards, AdaptiveRecommendation, SkeletonLoader, VisualLessonCard, InteractiveExercise, MultimodalContent, LessonImages, SleepMeditationHub)
- **Строк кода:** +2,500
- **Анимаций:** 25+ micro-interactions
- **Принципов Perplexity:** 10/10 применено
- **iOS 26 элементов:** 100% соответствие
- **Демо-кнопок:** 5/5 полностью функциональны

### **🎯 Все демо-компоненты работают:**

- ✅ **🎨 Визуальные уроки** - VisualLessonCard с эмоциональной адаптацией
- ✅ **🎮 Интерактивные упражнения** - InteractiveExercise с культурным контекстом
- ✅ **🎵 Мультимодальный контент** - MultimodalContent (текст + аудио + визуал)
- ✅ **🖼️ Картинки к урокам** - LessonImages как в Google Learn Your Way
- ✅ **💤 Сон и медитация** - SleepMeditationHub с интеграцией Calm + Headspace

### **🌐 Деплой статус:**

- ✅ **Production:** https://johnda7.github.io/da-teens-webapp-tele/
- ✅ **Локальная разработка:** Автоматически на свободном порту (обычно 5000 или 5001)
- ✅ **GitHub Actions:** Автоматический деплой успешен
- ✅ **Build:** Без ошибок, оптимизирован

**🎯 Все демо-компоненты работают на продакшене:**
- 🎨 **Визуальные уроки** - показывает VisualLessonCard с красивой заставкой
- 🎮 **Интерактивные упражнения** - показывает InteractiveExercise с ролевой игрой  
- 🎵 **Мультимодальный контент** - показывает MultimodalContent с аудио плеером
- 🖼️ **Картинки к урокам** - показывает LessonImages компонент
- 💤 **Сон и медитация** - показывает SleepMeditationHub компонент

**📱 Мобильная оптимизация (коммит 65af6fc):**
- ✅ Полная мобильная оптимизация для Telegram Mini App
- ✅ Touch-friendly интерфейсы (44px+ touch targets)
- ✅ Safe area support для мобильных устройств
- ✅ Адаптивный дизайн для разных размеров экранов
- ✅ Улучшенная интеграция с Telegram WebApp SDK
- ✅ Оптимизированная навигация для мобильных устройств
- ✅ Мобильно-оптимизированные карточки и кнопки
- ✅ Плавные анимации с hardware acceleration
- ✅ Поддержка viewport changes и touch событий

**✨ Реализованные улучшения:**
- ✅ Добавлены красивые заставки с эмодзи вместо отсутствующих изображений
- ✅ Убран текст "Русский акцент" из всех компонентов
- ✅ Заменены английские слова на русские (Video → Видео, russian → русский)
- ✅ Полностью русскоязычный интерфейс
- ✅ Современный дизайн в стиле iOS 26 Liquid Glass

---

## 🗺️ **ДОРОЖНАЯ КАРТА И БИЗНЕС-МОДЕЛЬ**

### **📅 Фазы развития:**
- **Q1 2025:** Telegram Mini App (MVP, тестирование)
- **Q2 2025:** Wellness Expansion (Calm + Headspace функции)
- **Q3 2025:** Global Preparation (локализация, монетизация)
- **Q4 2025:** Global Launch (App Store + Google Play)
- **2026:** Scale & Innovation (AI, социальные функции)

### **💰 Бизнес-модель:**
- **Freemium:** 70% пользователей (базовый контент бесплатно)
- **Premium Subscriptions:** 25% пользователей ($9.99/месяц)
- **In-App Purchases:** 5% пользователей ($0.99-$2.99)

### **📊 Финансовые цели:**
- **Q4 2025:** $10,000 MRR, 50,000 пользователей
- **2026:** $100,000 MRR, 500,000 пользователей
- **Целевая оценка:** $1B к 2028 году

### **🌍 Глобальная стратегия:**
- **Рынки:** США, Великобритания, Канада, Австралия, Германия, Франция
- **Локализация:** 5+ языков, культурная адаптация
- **Платформы:** iOS, Android, Web App, Telegram Mini App
...
## 📱 **TELEGRAM MINI APP: ЛУЧШИЕ МИРОВЫЕ ПРАКТИКИ**

### **🎯 Философия Telegram приложений:**

**1. ⚡ МГНОВЕННАЯ СКОРОСТЬ (Perplexity принцип)**
- Загрузка < 1 секунды
- Первый контент < 500ms
- Плавные 60fps анимации
- Минимум HTTP запросов

**2. 🎨 МИНИМАЛИЗМ И ЭЛЕГАНТНОСТЬ**
- Чистый белый фон (#FFFFFF)
- Тонкие границы (1px, #E5E5E5)
- Круглые кнопки (8px border-radius)
- Компактные отступы (8px grid system)

**3. 📱 МОБИЛЬНАЯ ПЕРВООЧЕРЕДНОСТЬ**
- Touch targets минимум 44px
- Swipe gestures для навигации
- Safe area для iPhone notch
- Вертикальная прокрутка

**4. 🌊 ПЛАВНЫЕ АНИМАЦИИ (НЕ УБИРАЕМ!)**
- Spring physics (stiffness: 300, damping: 30)
- Micro-interactions на каждое действие
- Stagger animations для списков
- Hover effects для desktop

### **🏆 ЛУЧШИЕ ПРИМЕРЫ МИРОВЫХ TELEGRAM ПРИЛОЖЕНИЙ:**

**1. Telegram Wallet (финансы)**
- Компактные карточки
- Круглые action buttons
- Минималистичный header
- Быстрые transitions

**2. Telegram Games (игры)**
- Полноэкранный режим
- Touch-оптимизированные элементы
- Быстрая загрузка контента
- Интуитивные жесты

**3. Telegram Shopping (покупки)**
- Карточки товаров
- Быстрый поиск
- Плавная навигация
- Четкие CTA кнопки
...
- Color palette (системные цвета Telegram)
...
### **🎨 ДИЗАЙН-СИСТЕМА TELEGRAM:**

**Цвета:**
- Primary: #007AFF (синий Telegram)
...
**Типографика:**
- Headers: SF Pro Display, 20-24px
- Body: SF Pro Text, 16-17px
...
**1. Сохраняем анимации, но оптимизируем:**
- Уменьшаем blur эффекты (40px → 20px)
- Убираем постоянные анимации (только по hover/tap)
- Используем will-change для GPU acceleration
- Debounce scroll events

**2. Улучшаем производительность:**
- React.memo для тяжелых компонентов
- useMemo для вычислений
- useCallback для функций
- Virtual scrolling для длинных списков

**3. Адаптируем под Telegram:**
- Используем Telegram theme colors
- Интегрируем haptic feedback
- Поддерживаем Telegram gestures
- Оптимизируем для маленьких экранов

---

**Последнее обновление:** Ноябрь 2025, 01:00 (портовая конфигурация обновлена)
